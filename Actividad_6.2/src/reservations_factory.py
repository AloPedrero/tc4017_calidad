...
class ReservationFactory:
    @staticmethod
    def create_reservation(hotel_id: str, customer_id: str):
        """
        ...
        try:
            ...
            if not check_if_id_exists("hotel", hotel_id) or not check_if_id_exists(
                "customer", customer_id
            ):
                raise Exception("Error:hotel/client with ID does not exists.")

            reservation_id = str(uuid.uuid4())
            reservation = Reservation(hotel_id, customer_id, reservation_id, "Reserved")

            df = pd.concat(
                [df, pd.DataFrame([reservation.to_dict()])], ignore_index=True
            )
    ...
    def cancel_reservation(reservation_id: str):
        ...
        try:
            ...
            if not check_if_id_exists("reservation", reservation_id):
                raise Exception(
                    f"Error: reservation with ID {reservation_id} dont exist."
                )
